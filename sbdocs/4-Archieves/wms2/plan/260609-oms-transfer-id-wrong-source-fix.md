...
> **Archive note (2026-07-16):** Implemented in `v2/oms-laravel-api` commit `1d4e46f` ("Fix contact-user sync and WMS transfer_id; add order-group parcels", 2026-06-22). `getTransferId` now builds `CLIENT_CODE-ORDER_ID` and the call site passes `$clientCode`; the implementation also closes the §8 multi-parcel gap by throwing when a transfer batch spans multiple orders. Acceptance script `sbdocs/9-System/scripts/verify-260609-oms-transfer-id-wrong-source-fix.sh` passed 4 pass / 0 fail (A5 phpunit check skipped — php not available in shell) and is retained for regression.

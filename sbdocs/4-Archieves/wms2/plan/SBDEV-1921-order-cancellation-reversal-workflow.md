...
**Priority:** high
**Status:** archived 2026-07-15 — WMS implementation complete and **merged directly to `develop` (no PR)**: wms2-api `bf14f6d` (merge of `tasks/SBDEV-1921`, verified on `origin/develop`) + `27c2cc7`; wms2-mobile-ui `c7f50bb`/`1cbfadd` (verified on `origin/develop`). The OMS-endpoint closure dependency (Q2/F1) is **not** part of this WMS plan and is tracked by the paired doc [[SBDEV-1921-oms-batch-reversal-completed-endpoint]]. **Update 2026-07-27: the OMS endpoint has shipped** — `POST /services/call/batchReversalCompleted`, delivered under ticket SBDEV-2369 (commit `b96b2daf`, 2026-06-10), now in production; Q2 resolved as a `parcel.reversal_completed_at` timestamp rather than a status enum. The only outstanding closure item is the per-env `WEBSERVICE_ORDER_BATCH_REVERSAL_COMPLETED` sysprop URL (still the `oms-XXXXX.siteboss.net` placeholder). Archived because the WMS work this plan owns is implemented and on `develop`; the ticket closes once that sysprop is set per environment.
**Date:** 2026-05-23
...
- [x] **OMS endpoint** `POST /services/call/batchReversalCompleted` implemented & deployed (Q2/F1). Tracked in [[SBDEV-1921-oms-batch-reversal-completed-endpoint]]. **DONE** — OMS shipped it under ticket **SBDEV-2369**, commit `b96b2daf` (2026-06-10), present on `develop` and `main` (production release v2.0.69); verified 2026-07-27. Q2 resolved in favour of a **`parcel.reversal_completed_at` timestamp** (not a status enum). The earlier "verified absent as of 2026-06-16" note on this line was **wrong** — the OMS commit predates that check by six days.
  - [ ] Still open before the ticket closes: set `WEBSERVICE_ORDER_BATCH_REVERSAL_COMPLETED` to the real per-env URL on each tenant DB (replaces the `oms-XXXXX.siteboss.net` placeholder in `WmsConstants.java:922`), and confirm whether the outbox dispatcher inspects the response `Status` field — the OMS handler returns **HTTP 200 with `{"Status":"Error"}`** for an unknown `batch_id`/`unique_id`, which a code-only check would mark `SENT`.

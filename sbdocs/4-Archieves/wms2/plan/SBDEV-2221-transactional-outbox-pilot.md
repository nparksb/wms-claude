...
db_verified_note: >
  ...
  V2.1.11. Pre-flight DB verification is a NEGATIVE check (table must not already
...
1. **Negative check (pre-flight):** confirm `outbox_message` does NOT exist in any tenant DB. If it does, this plan must not run — abort and investigate which prior commit introduced it.
   ...
   ```
2. **Negative check (Flyway):** confirm `V2.1.11` does not already exist in `src/main/resources/db/migration/`. The verify script (§verify script) checks this.
...
### Fix A — Flyway migration `V2.1.11__add_outbox_message.sql`
...
| File | Role |
|---|---|
| `src/main/resources/db/migration/V2.1.11__add_outbox_message.sql` | Table + indexes + check constraint |
...
|---|---|---|---|
| 1 | `db/migration/V2.1.11__add_outbox_message.sql` | NEW | ~30 |
...
- [ ] Confirm `V2.1.11` is unused: `ls v2/wms2-api/src/main/resources/db/migration/ | grep V2.1.11` returns nothing.
...
- Add `V2.1.11__add_outbox_message.sql`.
...
- Commit: `feat(wms2): add outbox_message table (SBDEV-2221) — Flyway V2.1.11`.
...
5. **Flyway version:** `V2.1.11` (next after V2.1.10).

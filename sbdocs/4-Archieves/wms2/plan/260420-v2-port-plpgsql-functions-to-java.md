...
> `sbdocs/4-Archieves/scripts/verify-260420-v2-port-plpgsql-functions-to-java-prekickoff.sh`.
...
>
> **OBSOLETE AS WRITTEN — the entire SQL baseline is gone.** History was squashed into `V2.2.00__base_v2_schema.sql` on 2026-07-17 (`cf82ad72`); **no `V1.*` or `V2.1.*` file remains on develop**, so every `V1.2.05:NNN` reference and the whole PR-#47 apparatus is dead. Flyway head is **V2.2.23**, next free **V2.2.24** (not V2.1.17). Three fixes landed in the functions since June — SBDEV-2777 (`V2.2.07`), SBDEV-2801 (`V2.2.08`), SBDEV-2890 (`V2.2.12`) — and all three precede any port **by fact**. Phase D would break four tests that pin the deployed bodies via `pg_get_functiondef`. Port target is **542 lines** of PL/pgSQL and unblocks exactly **one** disabled test method. **Recommendation: do not start.**

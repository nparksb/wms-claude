...
type: "bugfix"
priority: "high"
status: "draft — pending review. ⚠️ AMENDED 2026-08-20 from SBDEV-2968's P3 re-derivation: row 0.B.16 `isUnitLoadIdValid` was mis-targeted at `WEB_UI_VIEW_STOCK_UNIT` and is CORRECTED to EXCLUDED — it is called by BOTH UIs (mobile `store/moveStock.js:178`, web `store/handlingUnits/stockUnits.js:212`), so a WEB_UI_* gate would 403 Move Stock for every mobile-only operator. Note 3 is ANSWERED: mobile does not call `bulkTransferStock`, so that one is web-only and may carry a plain gate. Stale cross-refs fixed: 2968 now holds V2.2.18 (not V2.2.17), and its shared-endpoint list is 17+ (not 5). Unchanged and still true: this plan gates ACTIONS only and annotates no web controller for view functions (§1.3), so the shared surface stays ungated from both directions after both plans ship."
project: [wms2]
version: v2
requester: "Nam Park"
created: 2026-08-16
...
| 0.B.16 | `GET /v3/stockUnit/isUnitLoadIdValid/{labelId}` | ⚠ named `getStorageLocationsForStockMovement` (`:615-616`) — see note 4 | ❌ none | 🔴 **CORRECTED 2026-08-20 → EXCLUDED, shared with mobile.** Was `WEB_UI_VIEW_STOCK_UNIT`. **It is called by BOTH UIs** — mobile `store/moveStock.js:178`, web `store/handlingUnits/stockUnits.js:212` — so a `WEB_UI_*` gate **403s Move Stock for every mobile-only operator**. Found by SBDEV-2968's P3 re-derivation (2968 §14.12); 2968's §0.B listed only five shared endpoints and this was not one of them, which is why it was mis-targeted here. Either EXCLUDE it or give it the cross-namespace **ANY-of** (`WEB_UI_VIEW_STOCK_UNIT` **or** `MOBILE_UI_VIEW_STOCK_TRANSFER`) — do not plain-gate it. |
...
3. **`bulkTransferStock` (0.B.12) is *not* on the §0.B shared list, but its single-item sibling is.** ✅ **VERIFIED 2026-08-20 (SBDEV-2968 P3): the mobile UI does NOT call `bulkTransferStock`** — zero matches across `wms2-mobile-ui` on `origin/develop`. So **`bulkTransferStock` is web-only and can carry a plain gate**, while its single-item sibling `transferStock` (0.B.11) stays EXCLUDED / ANY-of. The asymmetry this note warned about is real and now resolved by measurement rather than assumption. ⚠️ Note the same check has now caught a *second* member of this trap in row 0.B.16, which was mis-targeted.
4. **Naming trap:** `GET /isUnitLoadIdValid/{labelId}` (`:615`) is served by a method literally *named* `getStorageLocationsForStockMovement` (`:616`) — a copy-paste leftover returning `Boolean` from a unit-load label lookup. **RESOLVED 2026-08-20:** both paths are now EXCLUDED (0.B.13 and 0.B.16), so name-matching happens to land correctly — but rely on the *path*, never the method name. Harmless at runtime, but it makes two different endpoints grep to the same method name. Do not use method names as the key when building the route map.
...
| 0.10 | `src/main/resources/db/migration/` | ⚠️ **2968 moved to V2.2.18 on 2026-08-20** (V2.2.17 was claimed by SBDEV-2994, `3abb1f2` — see 2968 §14.8); this plan takes the next free above that (**re-sweep at PR time; the answer is perishable**) |
...
**Out of scope:** the shared endpoints — ⚠️ **17+, not 5** (SBDEV-2968 §0.B as corrected by §14.12 on 2026-08-20; the count includes five Spring Data REST searches), the `AdminController` alias surface (§0.C of 2968 / SBDEV-2870), the audit-comment defect (§0.C above), and the 8 `WEB_UI_VIEW_*` constants with no page (§10.2).
...
**F2** — Flyway migration for provisioned tenants: `INSERT … WHERE NOT EXISTS` per (role, function), keyed by role **name**, skipping silently where a role is absent. `mywms_role_mywms_function` — ⚠️ **rationale corrected 2026-08-20 (SBDEV-2968 §14.8/P5): the conclusion `NOT EXISTS`, never `ON CONFLICT` is RIGHT, but not for the stated reason.** Measured: the base dump (`V2.2.00__base_v2_schema.sql:1512-1515`) has no unique constraint, **but both live tenants carry a composite PK under drifting names** — `…_pkey` on WineCo dev, `…_pk` on Hydra UAT. So `ON CONFLICT ON CONSTRAINT <name>` breaks on the name drift and column-inference `ON CONFLICT (rolelist_id, functionlist_id)` raises **42P10** on base-dump tenants: each form is wrong on a *different* subset, and `NOT EXISTS` is the only universally safe guard. **Do not "fix" this to an ON CONFLICT on the grounds that a constraint exists.** Version = next free after 2968's **V2.2.18** (corrected — 2968 lost V2.2.17 to SBDEV-2994); **re-sweep all remote branches at PR time**.
...
| **P5** | Re-sweep `V2.2.*` across all remote branches at PR time (**2968 now holds V2.2.18, not V2.2.17** — corrected 2026-08-20). | **YES** |
...
| **R8** | Flyway version collides with 2968's **V2.2.18** or an unmerged branch. ⚠️ **This risk already materialised once on 2968** — it reserved V2.2.17 on 08-17 and lost it to a sibling ticket on 08-19. | Medium | P5 re-sweep at PR time, immediately before the PR | Low **only at the moment of the sweep**. |
...
## Scope addition requested 2026-08-19 (from SBDEV-3005's code review)

**Gate the user-administration write endpoints server-side.** `POST /v3/userRole/saveRoleFunctions` and `POST /v3/userGroup/saveGroupRoles` are ungated writes to the access-control tables, so any `wms_user` can self-escalate: create a role → grant it `WEB_UI_VIEW_USER_MANAGEMENT` → attach it to a group they belong to → they then satisfy `denyUnlessUserManagementAllowed()` and reach everything SBDEV-2870 gated.

**Why here.** `/v3/**` is gated only by `hasAnyAuthority("wms_user")` (`SecurityConfiguration.java:151`), and `UserRoleController` / `UserGroupController` carry no function check — unlike `UserController:311`, which SBDEV-2870 gated. SBDEV-2968 §13.2 states its mechanism is platform-wide while its *coverage* is not, and names this plan as the widener. Neither plan currently mentions these endpoints: a grep for `saveRoleFunctions`, `saveGroupRoles`, `UserRoleController` and `UserGroupController` returns **zero hits in both**. So this is a genuine scope addition, and cheap while this plan is still in draft.

**Size:** roughly two `@RequiresFunction` annotations once SBDEV-2968 lands `@RequiresFunction` + `FunctionGuardInterceptor`. Blocked on that.

**Suggested function:** `WEB_UI_VIEW_USER_MANAGEMENT`, matching the tab these screens live under (§3.4 Fix D) and `UserController`'s existing guard.

⚠ **This is not sufficient on its own, and should not be presented as closing the escalation.** Two Spring Data REST surfaces write the same table without passing through any controller — `POST /v3/userRoleUserFunction` and the `PUT /v3/userRole/{id}/functions` association endpoint. Tracked separately as **SBDEV-3013**, because suppressing them is a published-API change. A guard added here closes one of three doors; SBDEV-3013 closes the other two.

⚠ **Sequencing:** SBDEV-3012 fixes a non-atomic delete-then-insert in these *same two methods*. Different defect, different acceptance criteria, so the tickets stay separate — but whoever goes second will be editing files the first just changed.

**Source:** SBDEV-3005 review (production lane), recorded in `SBDEV-3005-role-function-composite-key-swap.md` §12.3 and §14.

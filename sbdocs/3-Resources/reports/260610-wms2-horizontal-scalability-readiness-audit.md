...
1. **"`scanPallet` drops the OMS palletized notification" — FALSE.** `MobilePalletizingService.java:230-245`: when no tx synchronization is active the `else` branch calls `manageOrderService.customerOrderPalletized(...)` synchronously; `OmsNotificationService.sendAfterCommit:85-99` likewise falls back to immediate `doSend`. Nothing is dropped. Residual truth: `scanPallet` is non-transactional, so order-state update (`:217`, via correctly-used OptimisticLockRetry in auto-commit) and unit-load transfer are not atomic with each other — pre-existing, LOW. **⚠ CLOSED 2026-09-17 by SBDEV-3398, and it was not LOW.** The non-atomicity was reachable with a single operator and no concurrency: `removeBOLPositionIfExists` committed a DELETE of the parcel's `billoflading_position` rows and `assertPalletNotAssignedToGate` then rejected the scan, so a rejected re-scan onto a gated pallet destroyed those rows while the parcel still shipped on `carrierunitload_id`. The writes now run in one boundary under three pessimistic row locks, and `OptimisticLockRetry` was deleted.

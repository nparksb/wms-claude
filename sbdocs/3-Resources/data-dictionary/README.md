...
| [wms2-sysprop-catalog.md](./wms2-sysprop-catalog.md) | What is each v2 `sysprop` key, its default, and which service reads it? All **124** constants. | Enabling a cron job for a new v2 tenant; configuring OMS callback URLs; Keycloak realm setup; sysprop appears in v2 logs |
| [260730-wms2-sysprop-current-value-census.md](../reports/260730-wms2-sysprop-current-value-census.md) *(report)* | What value does each sysprop actually hold in each DEV/UAT tenant right now? Which keys are orphaned, junk, or missing? | Auditing tenant config; before changing a sysprop; onboarding a tenant. **Point-in-time — re-run, don't trust an old copy.** |

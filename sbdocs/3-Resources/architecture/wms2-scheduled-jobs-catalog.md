...
| `JobLockId.OUTBOX_DISPATCHER_CLUB` | `100009L` | `OutboxDispatcherJob` club lane |
| `JobLockId.PENDING_REVERSAL_RECONCILE` | `100010L` | `PendingReversalReconciliationJob` (SBDEV-3321) |

⚠ **The high-water mark is `100010L`, so the next free id is `100011L`.** This table omitted the club lane until 2026-09-16: it landed after the 2026-09-06 verification pass, so a reader taking "next free" from this doc would have allocated `100009L` and silently shared a mutex with the club dispatcher. Re-derive from `AdvisoryLockService.JobLockId` before allocating, not from this table. **This line has now been wrong twice** — it said `100009L` while the club lane held it, and was corrected to `100010L` hours before SBDEV-3321 took that one. A number written here is stale the moment the next job merges; the grep is the answer, this table is a convenience.
...
| **Wired** | `@Scheduled(cron = "${app.cron.outbox-dispatcher:*/3 * * * * *}")` — Spring `@Value` driven (not a DB sysprop) |
| **Lock ID** | `JobLockId.OUTBOX_DISPATCHER` (100008L — added by SBDEV-2221) |
| **Cron** | `app.cron.outbox-dispatcher=*/3 * * * * *` (every 3 s — tightened from the original 15 s; re-derive from `application.properties` rather than trusting this cell) |
...
| `OutboxDispatcherJob` (SBDEV-2221) | 100008L | `*/3 * * * * *` (app.properties) | **none — ungated, every replica** | none (no DB sysprop gate) | REQUIRES_NEW per phase (claim, mark*, cleanup) | per-tenant |
...
app.cron.outbox-dispatcher=*/3 * * * * *        # CRON EXPRESSION, gates nothing
```
...
| Task | Start at |
|---|---|
| Add a new cron job | §2 (reserve next lock ID — `100001L`–`100010L` are **all taken**, so the next free is `100011L`; and decide the **lock form** deliberately, see §2's per-job lock-shape table), §4 (copy the skeleton), §3.1 (wire in `SchedulingConfiguration`), §7 items 1/4/5 |
...
| 2026-09-16 | **Lock-id drift that would have caused a real collision, found while scoping SBDEV-3321's F5 job.** §2's lock-id table omitted `OUTBOX_DISPATCHER_CLUB = 100009L` (it landed after the 2026-09-06 pass), and §9's quick-reference row stated the next free id as `100009L` — so an implementer following this doc would have allocated a lock already held by the club dispatcher, silently sharing a mutex. Added the missing row, corrected the next-free to `100010L`, and added a pointer to re-derive from `AdvisoryLockService.JobLockId` rather than from this table. Separately, **`app.cron.outbox-dispatcher` is `*/3`, not `*/15`** — a 5× tick-rate error that feeds the §7 pool arithmetic; corrected at the three sites asserting it as the current value (§4.8 Wired, §4.8 Cron, §7.7's annotated block) and deliberately NOT at the two changelog rows (2026-05-17, 2026-06-01), which are accurate as history. | 4 sites corrected across 2 defects | Re-derived from `origin/develop` @ `e113467b`: `AdvisoryLockService.JobLockId` and `application.properties` read via `git show`. **NOT re-verified this pass:** everything else in the doc — this was a targeted two-defect fix, not an audit, so `last_verified` is deliberately left at 2026-09-06. |

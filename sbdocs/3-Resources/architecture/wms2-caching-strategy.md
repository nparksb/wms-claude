...
**Out-of-band cache (KeycloakService):** `KeycloakService` maintains a separate, manually managed Caffeine cache (`userCache`) that is NOT part of `CacheConfig` and NOT controlled by Spring Cache annotations. It caches `UserRepresentation` objects from Keycloak.
...
| `SkuRestController.delete` does not evict `itemdata` | `itemdata` | `DELETE /rest/sku` | Low: deleted SKUs in cache will cause `getById`/`findByClientIdAndItemNr` to return a stale entity that no longer exists in DB; callers must handle `EntityNotFoundException` defensively |

...
| 2026-09-10 | SBDEV-3244: `ReplenishmentOrderMaintenanceService.recalculateOrder` signature changed to **`(Long, RecalcContext)`** — it now loads the order itself with `findByIdForUpdate` as its first statement **that touches the row** (a `orderId == null` guard precedes it), and both the state and `manuallyoverridepriority` guards moved under that lock. `RecalcContext` no longer carries `Stockunit`; `ensureValidSource` and `redirectSource` load stock units with `findByIdForUpdate`. Both sibling probes (`reassignOrCancelForMovedStockUnit`, `ReplenishmentOrderSourceSyncService.syncForMovedStockUnit`) read an **id** via the new `findIdByStateLessThanAndStockunitId`. **No annotation, propagation or REQUIRES_NEW count changed** — this is a persistence-context change, not a transaction-boundary change. ⚠ The 2026-05-20 row above remains accurate about *why* the inner loop keeps a plain `this.` call, but do NOT read it as the mechanism of SBDEV-3244: that `UnexpectedRollbackException` came from `findByIdForUpdate` throwing `StaleObjectStateException` on a version check (`EntityInitializerImpl.upgradeLockMode` → `checkVersion`), after which **Hibernate's own** `ExceptionConverterImpl` called `markForRollbackOnly()`. No Spring proxy participated, so no `rollbackFor` reshaping could have fixed it. ⚠ Residual, NOT fixed: the two `@Transactional` callers (`StockunitService.setLockOnHold`, `FixLocationAssignmentService.move`) pre-load the same rows in the transaction the recalc joins, so the throw relocates to `ensureValidSource` there — SBDEV-3286 owns it. | 5 new id projections all `@RestResource(exported = false)`; post-merge **surefire 6412 / 0 fail / 0 err / 1 skip, failsafe 385 / 0 / 0 / 31**, against a measured `origin/develop` @ `22d19474` baseline of 6405 and 381 — i.e. +7 and +4 tests, zero new failures, skips unchanged. ⚠ An earlier draft of this row said "4 pre-existing `OrderReleaseSectionQueryIT` errors unchanged vs baseline": both halves were wrong — SBDEV-3285 (`00337dcb`) fixed those four on develop while this was in review, so the merge target is fully green, and dropping the skip column overstated the result. AC-3′ pins mutation-checked with attributable kills | SBDEV-3244 A-narrow implementation |

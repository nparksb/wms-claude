---
title: WMS v1 Replenish Workflow
type: workflow
project: wms1
status: stable
created: 2026-04-26
...
- ```finishReplenishmentOrder``` closes the loop: it reloads the order, revalidates source stock and destination, clears any reserved quantities, ensures the destination has an assigned unit load (creating one if missing), and transfers **all** stock that remains on the reserved unit load (because ```amountPicked``` is still null when the controller reloads the DTO) onto that unit load before marking the order FINISHED and triggering another ```refillFixedLocations()``` run. The mobile controller calls this immediately after a successful ```checkDestination```, so users do not invoke a separate “finish” step [(src/main/java/net/aim_ai/wms/service/mobile/MobileReplenishService.java (lines 359-425); src/main/java/net/aim_ai/wms/controller/mobile/ReplenishController.java (lines 180-200))](/src/main/java/net/aim_ai/wms/service/mobile/MobileReplenishService.java#L359-L425). **Implementation note (260427 fix):** `finishReplenishmentOrderInternal` calls `StockunitBusinessService.changeReservedAmount` twice to release the source reservation — once for the normal branch (L420) and once for the split branch (L424). Both calls rebind `sourceStock` to the return value (`sourceStock = changeReservedAmount(sourceStock, …)`) because `changeReservedAmount` detaches the caller's reference before acquiring the pessimistic lock; reading `getAvailableamount()` from the old reference would return 0 and cause `transferStockToUnitLoad` to throw.

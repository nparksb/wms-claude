---
title: WMS v2 Replenish Order Creation
type: workflow
project: wms2
status: stable
created: 2026-04-19
...
- **Scheduled pipeline** – `ReplenishOrderJob` runs via cron on the `runFor(TriggerSpec)` path (all tenants whose own schedule matches the firing trigger) or on demand via `runForCurrentTenant()` (the caller's tenant only). ⚠ **Updated 2026-09-06 (SBDEV-3198):** the single `doCalculation(Boolean isCronJob)` entry point this bullet used to name was **deleted** 2026-09-03 and the `isCronJob` flag no longer exists. Both paths check `SYSTEM_PROPERTY_NEW_CRON_JOB_ACTIVATED_KEY` and `SYSTEM_PROPERTY_REPLENISHMENT_TIMER_ACTIVATED_KEY` per tenant and skip that tenant if either is disabled (`ReplenishOrderJob.java:291-292` on the scheduled path, `:391-392` on the manual path, at `d4a6ab8a`). Once enabled, the private `replenish(String tenantName)` (`:415`) executes the same fixed sequence as before (`mergePickingOrders`, `deleteEmptyFixAssignmentWithoutStockToReplenish`, `cancelUnreachableReplenishment`, `cancelReplenishmentIfFlowbinIsFull`, `generateReplenishmentForItemDataWithoutFixedAssignment`, `generateReplenishmentForItemDataWithFixedAssignmentWithOrders`, `triggerRegularReplenishment`, `updateReplenishmentOrderPriority`, `recalculateReplenishmentOrderWithoutFixedLocationAssignment`) — the body was extracted **verbatim**, so every step below is unchanged. The steps that actually create orders are described below.
...
- **Fixed locations needing refill** – `FixLocationAssignmentRepository.getRefillFixedLocations` returns assignments where (1) the unit load linked to the assignment holds less stock than the assignment’s lower bound, (2) the assignment is active, (3) no open `Replenishorder` already references the same destination or item — since SBDEV-3153 this is two separate `NOT EXISTS` clauses rather than one with an `OR`, which is equivalent but lets Postgres use an index, and (4) there exists at least one unlocked (entity-lock = 0) stock unit for the item in an area that allows replenishment (`src/main/java/net/aim_ai/wms/repo/jpa/FixLocationAssignmentRepository.java`, the `REFILL_ELIGIBILITY_FROM_WHERE` constant). `ReplenishGeneratorService.refillFixedLocations` consumes that list, loads the assigned unit load, calculates the missing amount as `upperBound - currentAmount`, and immediately calls `calculateOrder` with the assignment’s item and destination (`src/main/java/net/aim_ai/wms/service/ReplenishGeneratorService.java:48-69`).

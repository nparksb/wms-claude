...
- [SBDEV-2507-web-palletize-already-truckloaded-guard.md](SBDEV-2507-web-palletize-already-truckloaded-guard.md) — WineCo ST#1023: the web Outbound-Report palletize (`ParcelMonitorViewService.palletise`) reuses an existing pallet with NO check that it's already assigned to a gate/BOL (the "already exists" throw is commented out at `:110-114`), so an operator added parcels to an already-truck-loaded pallet → outbound/BOL state corruption. Mobile guards this (`MobilePalletizingService:198-201` "Pallet already assigned to gate!"). Fix = shared `BillofladingPositionService.assertPalletNotAssignedToGate()` called by web `palletise` (+ `palletiseAndTruckLoad`) and routed through by mobile's 4 inline copies so they can't drift. Root cause is code-level/data-independent (WineCo DB likely has manual fixes). closeBOL finalize + data remediation OUT of scope. db_verified (code read). v1; v2 pairing TBD

...
- [SBDEV-3266-oms-message-rows-lost-in-aftercommit.md](SBDEV-3266-oms-message-rows-lost-in-aftercommit.md) — **draft** (ShipItEz W1 order 121603967; affects WineCo too). `MessageService.createServiceLog` has no `@Transactional`, so every `Message` row written from a `TransactionSynchronization.afterCommit()` callback is silently discarded — six-month outage of the WMS→OMS picking audit trail, DB-proven on both prod tenants via three deploy-tag cliffs (v1.26.15 / v1.26.17 / v1.26.38) and 30 217 burned message numbers. ⚠️ **The POST still fires** — audit-trail outage, not delivery outage. Fix is NOT `REQUIRES_NEW` on `createServiceLog` — that method has **zero external callers** (all 3 call sites are self-invocations, proxy bypassed), so the annotation would be **inert**. Chosen fix routes `ManageOrderService`'s 10 `createMessage` calls through the existing `createMessageInNewTransaction`; 9 non-deferred callers are affected too, but only 3 of them hold a transaction, and those run in a scheduled job. Rev 3 after two review passes. Backfill deliberately rejected. 🚦 gate: is v1 ShipItEz W1 worth fixing given the v2 cutover?

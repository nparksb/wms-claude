...
| 3 | **`readOnly = true`** on the read facade; the resolver must not be `readOnly` and must not open `REQUIRES_NEW` | **BY DESIGN — re-verify post-implementation** | `describeForSku` is a pure read. Safe **only** because 2732 §3.4a reads tier 3 via `SyspropRepository` and never `SyspropService.getStringDefault` (which INSERTs at `SyspropService.java:234`). 2643 adds no `REQUIRES_NEW` and no resolver of its own. If the guard on this invariant is ever deleted this becomes a write-in-readOnly bug — ANSWERED 2026-08-30: `neverCallsGetStringDefault` was replaced by `PutawayResolverSyspropIsolationArchTest`, which covers strictly more routes (SBDEV-3170) |

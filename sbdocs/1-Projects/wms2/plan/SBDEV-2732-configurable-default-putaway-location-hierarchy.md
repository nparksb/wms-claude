...
> **every real `sb_admin`, on every tenant, permanently**, independent of `KEYCLOAK_CLIENT`.
>
...
> matrix §2.1's precondition box. H1b was not
...
>
> 🔴 **CONSIDERED AND REJECTED 2026-08-26.** SBDEV-3017 §8.13 proposed exactly this and was retired the same day: the web UI gates the screen on **functions** (`require-function.js`) and `wms_admin` confers **zero**, so the disjunction sits on a different axis from the one granting access — and under OR-semantics that failure is **silent**. `wms_admin` is neither a `mywms_role` nor a `mywms_group` (0/0 on `dev_wh01_om1`). Both helpers have zero callers and should be **deleted**. Correct mechanism: a `FunctionEnum` function granted to `super-admin`. See role matrix §2.1 and SBDEV-3017-B1 §8.15.

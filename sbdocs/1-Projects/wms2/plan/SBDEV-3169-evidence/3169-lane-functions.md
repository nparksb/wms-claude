...
**6. `MethodSecurityConfig:9` enables `securedEnabled` and `jsr250Enabled` for zero users.** There is not one `@Secured`, `@RolesAllowed`, `@DenyAll` or `@PermitAll` in `src/main/java`. Dead configuration, but it also means those are live mechanisms an SDR rule source could be defeated by if someone adds one later. — **CLOSED 2026-09-01 by SBDEV-3156:** both flags are now `false` and pinned, and `MethodSecurityAnnotationSurfaceArchTest` bans all four annotations from `src/main` (direct or meta-annotated), so an SDR rule source can no longer be defeated this way. **A 3169 implementer does not need to defend against these** — the mechanisms-table cell at the top of this file (`@PostAuthorize`/`@Secured`/`@RolesAllowed`/`@DenyAll`/`@PermitAll`) should be read the same way.

...
**V2 Status:** Not yet applied — this plan covers the V2 port
...
| Migration numbering | `V1.1.09` | Latest is `V1.1.13` | New migration: `V1.1.14` |
...
7. **Constants + migration** — `WmsConstants` constants + `V1.1.14` Flyway migration seeding `VERTICAL`
...
**File:** `src/main/resources/db/migration/V1.1.14__add_pick_path_direction_sysprop.sql`

```sql
-- SBDEV-2096: Add configurable pick path direction system property
-- ID 145 continues the sequence established by V1.1.13 (ids 142/143/144).
...
> **Migration notes:**
> - `V1.1.13` (`stale_club_batch_cleanup_sysprops`) is the current latest (ids 142/143/144). This migration uses id 145.
...
> - `groupname = 'Operation Options'` matches the v1 migration and the WMS admin UI grouping for operational settings. V1.1.13 used `'Backend'` for infrastructure-level sysprops — pick path direction is operator-facing, so `'Operation Options'` is correct here.
...
| `db/migration/V1.1.14__add_pick_path_direction_sysprop.sql` | **New** | Seeds `PICK_PATH_DIRECTION = 'VERTICAL'` (id=145); `ON CONFLICT (client_id, syskey, workstation) DO NOTHING` |
...
- `V1.1.14__add_pick_path_direction_sysprop.sql` — INSERT `PICK_PATH_DIRECTION = 'VERTICAL'` (id=145, `ON CONFLICT (client_id, syskey, workstation) DO NOTHING`)
...
|------|--------|------------|
| Migration ID conflict | Medium | `V1.1.14` follows `V1.1.13`. Verify no conflicting migration exists before running |

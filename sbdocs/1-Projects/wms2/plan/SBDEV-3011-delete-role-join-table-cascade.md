...
Groups: admin, inbound-leads, night-shift, ops-managers, receiving-team, <11 more names>. Users: none.

⚠ SUPERSEDED BY SBDEV-3031 (2026-08-29): the cap moved 5 -> 20, so all 16 names now render and
there is NO `(and N more)` suffix on this case. The example above showed the truncated 5-name form.
```

| Rule | Value |
|---|---|
| Entries listed per holder kind | at most **`HOLDER_NAMES_IN_MESSAGE`** — the cap counts *rendered entries*, whether a resolved name or an `id <n>` fallback, so the message length is bounded regardless of how many holders are unnameable. **Was 5; SBDEV-3031 raised it to 20.** Surveyed across all five v2 tenant databases 2026-08-29: **five** roles exceeded the old cap (16, 12, 10, 9, 6) on **four of the five databases** — worst case `super-admin` at 16 on UAT hydra. An earlier draft of this row said "only ONE role on either tenant"; that was a 2-of-5 sample, and wrong |
...
| Total length | bounded by `HOLDER_NAMES_IN_MESSAGE` entries per kind plus the two `(and N more)` suffixes — **322 chars measured** at the real worst case (UAT hydra `super-admin`, 16 groups), ~494 at 20 names of the longest length that exists anywhere (17). An earlier draft said "~700 chars", which matched no scenario. ⚠ The cap bounds the entry COUNT, not the length: `varchar(255)` names permit ~5.2 KB groups-only / ~10.4 KB both sides. Still bounded, but not by characters |
...
| `refusalMessageTruncatesAtTheHolderCap` | same (new in iteration 2) | **AC-3** — 16 holders in, at most 5 names plus `(and 11 more)` out. Pins the cap so the `super-admin` case has a definite expectation. ⚠ **SBDEV-3031**: this row never matched the code — the 16-holder assertion shipped inside `refusalMessageNamesTheHoldingGroups`, which now expects all 16 names. The bounding property moved to the new `refusalMessageStillTruncatesAboveTheRaisedCap` (25 holders -> 20 + `(and 5 more)`) | **ERRORS** |
...
| M2 | **The blast-radius case — no partial destruction, and the message is bounded** | same | Same against role `50356` (`super-admin`, 77 grants, 16 groups) | 422; `count = 77` unchanged. Body matches §4.3's `describeHolders` contract exactly: `held by 16 group(s) and 0 user(s)`, the group names, then `Users: none`. A bare count is a **FAIL**. ⚠ **REVISED BY SBDEV-3031**: this row used to require **at most 5 names** then `(and 11 more)`, and declared a 16-name message a FAIL. At `HOLDER_NAMES_IN_MESSAGE = 20` **all 16 names with no suffix is the CORRECT result** — the old wording would now reject correct code. **This is the end-to-end evidence for AC-2** | |
...
| **AC-3** | The 422 body follows §4.3's `describeHolders` contract: both holder counts, at most `HOLDER_NAMES_IN_MESSAGE` names per kind with an `(and N more)` suffix, `none` for an empty kind, and `fieldName = "roleId"` | `refusalMessageNamesTheHoldingGroups`, **`refusalMessageStillTruncatesAboveTheRaisedCap`** (SBDEV-3031; replaces the never-existent `refusalMessageTruncatesAtTheHolderCap` this row used to cite), `refusalSurfacesAs422NamingTheHolder`; verify `A8`/`T2`; M1, M2. ⚠ **The §11 caveat has EXPIRED**: SBDEV-3030 shipped (`wms2-web-ui cdaf9b0`), so the body now reaches the operator — but only as a 4.5 s toast, which SBDEV-3031 covers |
...
| §13.2 name | Actual |
|---|---|
| `refusalMessageTruncatesAtTheHolderCap` | folded into `refusalMessageNamesTheHoldingGroups` (now a full-message equality assertion). ⚠ **SBDEV-3031 supersedes this row**: that test no longer truncates at all — it asserts all 16 names — and the bounding property now lives in the new `refusalMessageStillTruncatesAboveTheRaisedCap`. See the §12 row above |

...
| [wms2-apply-pending-tenant-flyway.md](./wms2-apply-pending-tenant-flyway.md) | wms2-api v2 / DEV + UAT + PRD | SEV3 (SEV2 on PRD) | Deploy step for every PRODUCTION promotion, every UAT `wms2-api:rc-*` release, and every DEV `develop` merge carrying a new `V2.2.x` — bring **every** tenant DB up to that environment's branch `db/migration` set (`develop` / `release` / `main`). Tenant list is discovered from the landlord, so newly onboarded clients are picked up automatically. Covers the required `--env dev\|uat\|prd` profile (each has a different landlord DB name, and PRD a different role `wms2_landlord_app`; DEV's is `dev_landlord`, *not* the stale `landlord` beside it), the PRD `--confirm-production` gate + behind-upstream refusal + the live-v1-databases co-tenancy hazard, the DEV auto-deploy ordering that removes the pre-deploy window, the `--status`/`--apply` driver script, the manual baseline path for DBs with no `flyway_schema_history` (V2.2.03 is a non-idempotent `ADD COLUMN`), the landlord `L00x` deploy gate that Flyway does **not** manage, and checksum-drift / partial-migrate recovery |
| [wms1-verify-move-stock-lost-update-on-dev.md](./wms1-verify-move-stock-lost-update-on-dev.md) | wms-api v1 | VERIFY | **Verification**, not remediation — proves the SBDEV-3003 Move Stock lost-update fix holds on WineCo v1 dev. Concurrent-replay test via the API (isolates the API fix from the UI guard, which deploy separately), UI double-tap, and the club-run case where 2+ orders split ONE shared staging stock unit — the scenario that killed two candidate fixes and which no automated test can cover (v1 IT lane dead, SBDEV-2384). Carries the full `PUT /rest/order/create` CLUB payload with every required field, the duplicate-removal detector for finding existing victims, and three traps that cost real round trips: `created` is warehouse-local while `now()` is UTC (empty result reads as "nothing happened"), `priority` is 1–5 on input not the stored scale, and `activateBatch` wants the numeric PK while `runClubLine` wants the batchid string |
| [SBDEV-2481-stale-pick-line-move-qa-checklist.md](./SBDEV-2481-stale-pick-line-move-qa-checklist.md) | wms-api v1 | QA checklist | Manual test plan for the SBDEV-2481 fix (PR #176) — verify pick lines realign (not-started) or moves block (active picking) on fixed-assignment / unit-load / stock / replenish / nirvana moves, plus outbound + split pass-through regression guards. Includes precondition setup, 11-scenario matrix, and SQL verification |

...
| [wms2-apply-pending-tenant-flyway-uat.md](./wms2-apply-pending-tenant-flyway-uat.md) | wms2-api v2 / UAT | SEV3 | Pre-deploy step for every UAT `wms2-api:rc-*` release — bring **every** tenant DB up to the `release` branch's `db/migration` set. Tenant list is discovered from `landlord.tenant_db_configuration`, so newly onboarded clients are picked up automatically. Covers the `--status`/`--apply` driver script, the manual baseline path for DBs with no `flyway_schema_history` (V2.2.03 is a non-idempotent `ADD COLUMN`), the landlord `L00x` deploy gate that Flyway does **not** manage, and checksum-drift / partial-migrate recovery |
